...
from ..common.settings import ENABLE_VISUAL, ENABLE_STACKING, OBSERVE_STEPS, MODEL_STORE_INTERVAL, GRAPH_DRAW_INTERVAL, SUCCESS
...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        self.max_his_len = 15  # Maximum history length
        self.state_dim = NUM_SCAN_SAMPLES + 4 # Assumes util provides state dimension
        self.EP_HS = np.zeros([self.max_his_len, self.state_dim])
        self.EP_HL = 0  # Current history length
        ...
        self.replay_buffer = ReplayBuffer(self.model.buffer_size, self.model.state_size, self.model.action_size)
        self.graph = Graph()
        ...
        while (True):
            ...
            while not episode_done:
                if self.training and self.total_steps < self.observe_steps:
                    ...
                    action = self.model.get_action(state, self.EP_HS, self.EP_HL, self.training, step, ENABLE_VISUAL)
                ...
                if self.training == True:
                    ...
                    if self.EP_HL == self.max_his_len:
                        self.EP_HS[:(self.max_his_len-1)] = self.EP_HS[1:]
                        self.EP_HS[self.max_his_len-1] = list(state)
                    else:
                        if self.EP_HL > 1:
                            self.EP_HS[-(self.EP_HL+1)] = list(state)
                        self.EP_HL += 1

                    # print(f"Replay Buffer Length: {self.replay_buffer.get_length()}")
                    ...
                    if self.replay_buffer.get_length() >= self.model.batch_size:
                        ...
                        # print(f"Loss Critic: {loss_c}, Loss Actor: {loss_a}")
            ...
            if self.episode >= 1600:
                print("Training finished after reaching 1600 episodes.")
                rclpy.shutdown()
                sys.exit()
