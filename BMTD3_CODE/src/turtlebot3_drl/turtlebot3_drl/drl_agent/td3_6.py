'''
                ...
                “           基于GRU - Attention的TD3网络移动机器人导航          ”
                        ...
                        TD3+GRU+多头注意力机制\多头自注意力机制+TCN

                        *状态和动作分别单独FC,然后合并进行FC,五层FC
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size, num_heads = 2):
        ...
        tcn_channels = [self.hidden_size, self.hidden_size, self.hidden_size]
        self.tcn = TemporalConvNet(num_inputs=self.hidden_size, num_channels=tcn_channels, kernel_size=2, dropout=0.2)

        self.hlstm = nn.ModuleList([nn.GRU(input_size=512, hidden_size=512, batch_first=True)])

        self.multihead_attention = nn.MultiheadAttention(embed_dim=512, num_heads=self.num_heads, batch_first=True)
    ...
    def forward(self, states, his_state, his_len, visualize=False):
        ...
        his_input = his_input.permute(0, 2, 1)  # (batch_size, hidden_size, seq_len)

        his_input = self.tcn(his_input)  
        his_input = his_input.permute(0, 2, 1)  
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size, num_heads = 2):
        ...
        # TCN
        tcn_channels_1 = [self.hidden_size, self.hidden_size, self.hidden_size]
        self.tcn_1 = TemporalConvNet(num_inputs=self.hidden_size, num_channels=tcn_channels_1, kernel_size=2, dropout=0.2)
        ...
        tcn_channels_2 = [self.hidden_size, self.hidden_size, self.hidden_size]
        self.tcn_2 = TemporalConvNet(num_inputs=self.hidden_size, num_channels=tcn_channels_2, kernel_size=2, dropout=0.2)
    ...
    def forward(self, states, actions, his_state, his_action, his_len):
        ...
        hs_1 = hs_1.permute(0, 2, 1)  # (batch_size, hidden_size, seq_len)
        hs_1 = self.tcn_1(hs_1)  # (batch_size, hidden_size, seq_len)
        hs_1 = hs_1.permute(0, 2, 1)  # (batch_size, seq_len, hidden_size)

        hs_2 = hs_2.permute(0, 2, 1)  # (batch_size, hidden_size, seq_len)
        hs_2 = self.tcn_2(hs_2)  # (batch_size, hidden_size, seq_len)
        hs_2 = hs_2.permute(0, 2, 1)  # (batch_size, seq_len, hidden_size)
