...
class OffPolicyAgent(ABC):
    ...
    def _train(self, replaybuffer):
        ...
        h_state = batch['h_state'].to(self.device) 
        h_action = batch['h_action'].to(self.device) 
        h_next_state = batch['h_next_state'].to(self.device) 
        h_next_action = batch['h_next_action'].to(self.device) 
        h_state_length = batch['h_state_length'].to(self.device) 
        h_next_state_length = batch['h_next_state_length'].to(self.device) 


        result = self.train(state, action, reward, next_state, done, 
                            h_state, h_action, h_next_state, h_next_action, 
