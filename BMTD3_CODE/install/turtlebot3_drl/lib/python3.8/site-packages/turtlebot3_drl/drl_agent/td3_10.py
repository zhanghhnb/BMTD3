...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_output, attn_output_weights = self.attention(x, x, x)  # Q, K, V are the same
...
class WeightedBiGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(WeightedBiGRU, self).__init__()
    ...
    def forward(self, x):
        ...
        output = gru_output[:, :, :self.hidden_dim]  # 
...
class Attention(nn.Module):
    def __init__(self, hidden_size, init_range=0.1):
        super(Attention, self).__init__()
        # 
        self.w_omega = nn.Parameter(torch.Tensor(hidden_size, hidden_size))  # 
        self.u_omega = nn.Parameter(torch.Tensor(hidden_size, 1))  # 
        nn.init.xavier_uniform_(self.w_omega, gain=1.0)  
        nn.init.xavier_uniform_(self.u_omega, gain=1.0)
    ...
    def forward(self, x):
        ...
        u = torch.tanh(torch.matmul(x, self.w_omega))  # [batch_size, seq_len, hidden_size]
        
        att = torch.matmul(u, self.u_omega)  # [batch_size, seq_len, 1]

        att_score = F.softmax(att, dim=1)  # [batch_size, seq_len, 1]
        
        scored_x = x * att_score  # [batch_size, seq_len, hidden_size]
        
        feat = torch.sum(scored_x, dim=1)  # [batch_size, hidden_size]
...
class Actor(Network):
    def __init__(self, name, state_size, action_size, hidden_size, num_heads = 4):
        ...
        self.hlstm = nn.ModuleList([WeightedBiGRU(input_dim=self.state_dim, hidden_dim=self.state_dim)])


        self.attention = MultiHeadAttention(self.state_dim, num_heads=num_heads)

        self.hfc2 = nn.ModuleList([nn.Linear(self.state_dim, self.h), nn.ReLU(), nn.Dropout()])
    ...
    def forward(self, states, his_state, his_len, visualize=False):
        ...
        for layer in self.hlstm: his_input = layer(his_input)



        x = his_input

        feat = self.attention(his_input)
...
class Critic(Network):
    def __init__(self, name, state_size, action_size, hidden_size, num_heads = 4):
        ...
        self.hlstm1 = nn.ModuleList([WeightedBiGRU(input_dim=self.state_dim, hidden_dim=self.state_dim)])

        self.attention1 = MultiHeadAttention(self.state_dim, num_heads=num_heads)


        self.h2 = nn.ModuleList([nn.Linear(self.state_dim, self.h), nn.ReLU()])
        ...
        self.hlstm2 = nn.ModuleList([WeightedBiGRU(input_dim=self.state_dim, hidden_dim=self.state_dim)])


        self.attention2 = MultiHeadAttention(self.state_dim, num_heads=num_heads)


        self.h4 = nn.ModuleList([nn.Linear(self.state_dim, self.h), nn.ReLU()])
    ...
    def forward(self, states, actions, his_state, his_action, his_len):
        ...
        for layer in self.hlstm1: hs_1 = layer(hs_1)
        for layer in self.hlstm2: hs_2 = layer(hs_2)


        feat_1 = self.attention1(hs_1)
        feat_2 = self.attention2(hs_2)
